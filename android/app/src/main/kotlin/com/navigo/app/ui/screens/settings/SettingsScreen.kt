...
fun SettingsScreen(onBack: () -> Unit) {
    ...
    var debugLog by remember { mutableStateOf(WidgetDebugLog.read(context)) }
    LifecycleEventEffect(Lifecycle.Event.ON_RESUME) {
        ...
        debugLog = WidgetDebugLog.read(context)
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            */
            ...
            // Diagnostics — temporary debug surface for the widget-binding
            // investigation. Mirrors the same events Log.d emits, persisted
            // to SharedPreferences so they survive process restarts and
            // capture events from the launcher's bind of the
            // RemoteViewsService. Remove this section once the widget is
            // stable.
            Spacer(Modifier.height(20.dp))
            HorizontalDivider()
            Spacer(Modifier.height(20.dp))
            ...
            Text(
                "Widget event log — newest first. Capture this and share it " +
                    "if the widget isn't rendering.",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            Spacer(Modifier.height(8.dp))
            Surface(
                ...
                Text(
                    debugLog.ifBlank { "(no events yet)" },
